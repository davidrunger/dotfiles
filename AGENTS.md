...
- Add a final standalone line to the first commit message on a branch in the form `` `codex resume [uuid]` ``, replacing `[uuid]` with the current Codex conversation UUID (use `CODEX_THREAD_ID` when it is available). Keep this line at the very bottom of the message, after all body paragraphs. Treat the resume line as literal commit-message text. Quote it safely or use a message file when creating the commit. Never let the backticks or command text be interpreted by the shell or execute the command.
