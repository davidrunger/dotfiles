...
- Before the branch has been pushed to a remote, keep its work in a single commit and amend that commit as needed. This lets GitHub automatically use that commit's title and body as the pull request title and description; pushing multiple commits initially prevents that.
- This single-commit requirement ends after the branch's first push. Later pushes may contain multiple new commits; unpushed follow-up commits may still be amended, but they do not need to be folded into one new commit.
...
- Format the first commit title on a branch as `[subject area] Imperative title [JIRA-123]` when there is a natural, useful subject area, or as `Imperative title [JIRA-123]` otherwise. A subject area is optional; do not invent or force one solely to satisfy the format. This title becomes the GitHub pull request title and the eventual commit title on `main`, where a useful subject area and any Jira issue provide context.
...
- When the first commit title includes a subject area, use it to name the area affected and the imperative title to describe the specific change within that area. The two parts should usually contribute complementary information rather than repeat each other. Repetition is acceptable when the subject area is the clearest identifier for the change and the imperative title naturally names the same thing, as in `[AGENTS.md] Add AGENTS.md`; otherwise, choose a complementary subject area when it adds useful context.
...
- Do not hard-wrap commit message body paragraphs. GitHub uses the body as the pull request description, where natural paragraph wrapping is preferable; when a pull request is squash-merged, GitHub will hard-wrap the description in the resulting commit on `main`.
...
- When an RSpec example does not have a description string, use `specify` rather than `it`, unless `it` introduces Ruby that reads naturally as English, such as `it { is_expected.to ... }`.
...
- Do not add a regression spec merely to confirm an implementation detail when the same assertion would not be written for a feature built from scratch. A migration can warrant such a spec when the path to the final state protects meaningful behavior that could otherwise be lost; confirming that a specific native element or library component is rendered is usually insufficient on its own.
